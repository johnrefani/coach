...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$isBanned) {
    ...
    elseif ($action === 'create_post' && isset($_POST['post_title'], $_POST['post_content'])) {
        ...
        if (!empty($postTitle) && !empty($postContent)) {
            $stmt = $conn->prepare("INSERT INTO chat_messages (user_id, display_name, message, is_admin, is_mentor, chat_type, title, file_path, file_name, user_icon) VALUES (?, ?, ?, ?, ?, 'forum', ?, ?, ?, ?)");
    ...
    elseif ($action === 'create_comment' && isset($_POST['comment_message'], $_POST['post_id'])) {
        ...
        if (!empty($commentMessage) && $postId > 0) {
            $stmt = $conn->prepare("INSERT INTO chat_messages (user_id, display_name, message, is_admin, is_mentor, chat_type, forum_id, user_icon) VALUES (?, ?, ?, ?, ?, 'comment', ?, ?)");
            $isAdmin = 0;
            $isMentor = 0;
    ...
    elseif ($action === 'delete_post' && isset($_POST['post_id'])) {
        ...
        if ($postId > 0) {
            ...
            $stmt = $conn->prepare("DELETE FROM chat_messages WHERE id = ? AND user_id = ?");
            $stmt->bind_param("ii", $postId, $userId);
            $stmt->execute();
        }
        header("Location: forums.php");
...
if ($postsResult && $postsResult->num_rows > 0) {
    while ($row = $postsResult->fetch_assoc()) {
        $comments = [];
        $commentsStmt = $conn->prepare("SELECT * FROM chat_messages WHERE chat_type = 'comment' AND forum_id = ? ORDER BY timestamp ASC");
