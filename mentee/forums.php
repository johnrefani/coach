...
if (!isset($_SESSION['user_type']) || $_SESSION['user_type'] !== 'Admin') {
    // If not a Admin, redirect to the login page.
...
// --- AUTO UNBAN CHECK ---
// Automatically unban users whose temporary ban has expired
$auto_unban_stmt = $conn->prepare("DELETE FROM banned_users WHERE ban_type = 'Temporary' AND ban_until IS NOT NULL AND ban_until <= NOW()");
$auto_unban_stmt->execute();
$auto_unban_stmt->close();
...
// Count total bans for this user (including expired ones)
$ban_count_stmt = $conn->prepare("SELECT COUNT(*) as total_bans FROM banned_users WHERE username = ?");
$ban_count_stmt->bind_param("s", $username);
$ban_count_stmt->execute();
$ban_count_result = $ban_count_stmt->get_result();
if ($ban_count_result->num_rows > 0) {
    $ban_count_row = $ban_count_result->fetch_assoc();
    $ban_count = $ban_count_row['total_bans'];
}
$ban_count_stmt->close();

// Check if user is currently banned
$ban_check_stmt = $conn->prepare("SELECT reason, ban_until, ban_type, ban_duration_text FROM banned_users WHERE username = ? AND ((ban_type = 'Permanent') OR (ban_type = 'Temporary' AND ban_until IS NOT NULL AND ban_until > NOW()))");
...
if ($ban_result->num_rows > 0) {
    ...
    $ban_type = $ban_details['ban_type'] ?? 'Temporary';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$isBanned) {
    ...
    elseif ($action === 'create_post' && isset($_POST['post_title'], $_POST['post_content'])) {
        ...
        if (!empty($postTitle) && !empty($postContent)) {
            ...
            // Initialize admin/mentor flags as this page is for Admins
            $isAdmin = 0;
            $isMentor = 0;
    ...
    <div class="chat-container">
        <?php if ($isBanned): ?>
            <div class="banned-message" style="text-align: center; background-color: #f8d7da; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                ...
                <div class="ban-info">
                    <p><strong>Total Times Banned:</strong> <?php echo $ban_count; ?></p>
                    <p><strong>Reason:</strong> <?php echo htmlspecialchars($ban_details['reason']); ?></p>
                    <p><strong>Ban Type:</strong> <span class="ban-type-badge <?php echo ($ban_type === 'Permanent') ? 'ban-type-permanent' : 'ban-type-temporary'; ?>"><?php echo htmlspecialchars($ban_type); ?></span></p>
                </div>
                
                <?php if ($ban_type === 'Temporary' && $ban_details['ban_until']): ?>
                    <div class="ban-info">
                        <p><strong>Ban Duration:</strong> <?php echo htmlspecialchars($ban_details['ban_duration_text']); ?></p>
                        <p><strong>Unban Date:</strong> <?php echo date("F j, Y, g:i a", strtotime($ban_details['ban_until'])); ?></p>
                    </div>
                    <div class="ban-countdown">
                        <strong>⏱️ Time Remaining:</strong> <span id="countdown" data-ban-until="<?php echo htmlspecialchars($ban_details['ban_until']); ?>">Loading...</span>
                    </div>
                <?php elseif ($ban_type === 'Permanent'): ?>
                    <div class="permanent-ban-warning">
                        ⚠️ This is a PERMANENT ban. You will not be able to access the forums.
                    </div>
                <?php endif; ?>
            </div>
        <?php endif; ?>
...
<script>
    ...
    function updateCountdown() {
        ...
        const banUntilText = countdownElement.getAttribute('data-ban-until');
        if (!banUntilText) return;

        const banUntilDate = new Date(banUntilText).getTime();
        const now = new Date().getTime();
        const diff = banUntilDate - now;
