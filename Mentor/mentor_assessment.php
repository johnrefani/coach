...
if (isset($_POST['final_submit']) && isset($_SESSION['mentor_signup_data']) && isset($_POST['assessment_score'])) {
    ...
    // Check if assessment state is still in session (means they might have refreshed results page and are trying to resubmit)
    // Although clearing session state after showResult() should prevent this, this is an extra layer.
    // A more robust check is the database check below.


    // Check if application for this username/course already exists to prevent duplicate submissions
    ...
    if ($username && $expertise) {
        ...
        $check_stmt = $conn->prepare("SELECT COUNT(*) FROM applications WHERE Applicant_Username = ? AND AreaofExpertise = ?");
         if ($check_stmt === false) {
        ...
        if ($check_result[0] > 0) {
            ...
            // Clear related session data as the application is considered complete (already exists)
            unset($_SESSION['mentor_signup_data']);
            unset($_SESSION['assessment_state']); // Also clear assessment state
    ...
    // Extract and sanitize data from the stored signup_data
    $fname = $signup_data['fname'] ?? '';
    $lname = $signup_data['lname'] ?? '';
    $dob = $signup_data['birthdate'] ?? '';
    $gender = $signup_data['gender'] ?? '';
    $email = $signup_data['email'] ?? '';
    $contact = $signup_data['full-contact'] ?? ''; // Use the full contact with +63
    $username = $signup_data['username'] ?? '';
    $password = $signup_data['password'] ?? ''; // This is the plain text password from the first form - HASH IT
    $mentored_before = $signup_data['mentored'] ?? '';
    $experience = $signup_data['experience'] ?? '';
    $expertise = $signup_data['expertise'] ?? '';
    $resume_path = $signup_data['Resume'] ?? null; // Get the path from session
    $certificates = $signup_data['Certificates'] ?? null; // Get the paths from session
    ...
    (First_Name, Last_Name, DOB, Gender, Email, Contact_Number, Applicant_Username, Applicant_Password, Mentored_Before, Mentoring_Experience, AreaofExpertise, Resume, Certificates, Assessment_Score, Status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $username, $hashed_password, $mentored_before,
