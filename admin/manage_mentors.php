    ...
    try {
        ...
        if ($sendgrid_api_key && $mentor_email) {
            try {
                ...
                $email_sent_status = 'Email exception error. Check PHP error log.';
            }
        }
        
        echo json_encode(['success' => true, 'message' => "Course assignment successfully removed! **$course_title** is now unassigned. Email status: $email_sent_status"]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'change_course') {
    ...
    try {
        ...
        // Send email notification for REASSIGNMENT ONLY
        $email_sent_status = 'Email not sent';
        $sendgrid_api_key = $_ENV['SENDGRID_API_KEY'] ?? null;
        $from_email = $_ENV['FROM_EMAIL'] ?? 'noreply@coach.com';
        ...
        if ($sendgrid_api_key && $mentor_email) {
            try {
                ...
                $change_text = $old_course_title 
                    ? "Your handled course has been reassigned from **$old_course_title** to a new course." 
        ...
        echo json_encode(['success' => true, 'message' => "Course assignment successfully updated to **$new_course_title**. Email status: $email_sent_status"]);
        ...
        if ($sendgrid_api_key) {
            try {
                ...
                <div class='container'>
                    ...
                    <div class='course-box'>
                        <p>You have been assigned to mentor the course: **$course_title**.</p>
                    </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'reject_mentor') {
    ...
    try {
        ...
        echo json_encode(['success' => true, 'message' => "Mentor application rejected. Email status: $email_sent_status"]);
...
<body>
    ...
    <script>
        ...
        function showCustomConfirm(title, message, callback) {
            ...
            okButton.removeEventListener('click', okButton.currentListener);

            // Add new listener
            okButton.currentListener = function() {
                closeCustomConfirm();
                callback(true);
            };
            okButton.addEventListener('click', okButton.currentListener);
        }
        
        function closeCustomConfirm() {
            document.getElementById('customConfirmModal').style.display = 'none';
            // Optionally, clear the listener from the OK button to prevent memory leaks/double-calls
            const okButton = document.getElementById('customConfirmOK');
            if (okButton.currentListener) {
                okButton.removeEventListener('click', okButton.currentListener);
                okButton.currentListener = null;
            }
        }
        ...
        function createMentorRow(mentor, isApplicant) {
            ...
            if (mentor.status === 'Under Review') {
                ...
                actionHtml = `
                    <button class="btn btn-view-reason" onclick="showCustomAlert('Rejection Reason', '${mentor.reason ? mentor.reason.replace(/'/g, "\\'") : 'No reason provided.'}')">View Reason</button>
                    `;
            }
        ...
        function searchMentors() {
            ...
            rows.forEach(row => {
                // Only process non-template rows
            ...
            if (noDataRow) {
                if (currentMentorData.length > 0) {
                     // If there's data in the current tab, but the filter finds nothing
                    ...
                    // If the tab is empty initially, keep the initial message and show it
                    noDataRow.innerHTML = `<td colspan="9" style="text-align: center;">No mentors in this category.</td>`;
                    noDataRow.style.display = '';
                }
            } else if (!found && currentMentorData.length > 0) {
                // If no 'no-data' row exists, but search yields no results (and we have data)
                const newNoDataRow = document.createElement('tr');
                newNoDataRow.classList.add('no-data');
                newNoDataRow.innerHTML = `<td colspan="9" style="text-align: center;">No results found for "${searchInput.value.trim()}".</td>`;
                mentorTableBody.appendChild(newNoDataRow);
            }
        }
            ...
            fetch(`manage_mentors.php?action=get_available_courses`)
                ...
                .then(data => {
                    ...
                    if (data && data.length > 0) {
                        ...
                        showCustomAlert('Warning', 'There are no unassigned courses available. A mentor must be assigned a course upon approval.');
                    }
                })
                .catch(error => {
                    console.error('Error fetching available courses:', error);
                    selectElement.innerHTML = '<option value="">Error loading courses</option>';
                    showCustomAlert('Error', 'Failed to load available courses. Please try again.');
                });
        }
        ...
        assignCourseForm.onsubmit = function(e) {
            e.preventDefault();
            const courseSelect = document.getElementById('availableCoursesAssign');
            if (courseSelect.disabled) {
                showCustomAlert('Error', 'Cannot approve: No courses available for assignment.');
                return;
            }
            ...
            showCustomConfirm(
                'Confirm Approval', 
                'Are you sure you want to **APPROVE** this mentor and assign the selected course? An email notification will be sent.', 
                (confirmed) => {
                    if (confirmed) {
                        ...
                        .catch(error => {
                            closeCourseAssignmentPopup();
                            showCustomAlert('Error', 'An unexpected network error occurred. Check console for details.');
        ...
        rejectMentorForm.onsubmit = function(e) {
            ...
            showCustomConfirm(
                'Confirm Rejection', 
                'Are you sure you want to **REJECT** this mentor application? An email notification will be sent.', 
                (confirmed) => {
                    if (confirmed) {
                        fetch('manage_mentors.php', {
                            method: 'POST',
        ...
        function openUpdateCoursePopup(mentorId, mentorName, oldCourseId, oldCourseTitle) {
            ...
            // If there's an assigned course, show the remove button
            if (oldCourseId && oldCourseTitle !== 'Unassigned') {
                btnRemoveAssignment.style.display = 'inline-block';
                // Also enable the form for changing the course
                document.getElementById('availableCoursesChange').disabled = false; 
                document.querySelector('#changeCourseForm button[type="submit"]').disabled = false;
            } else {
                // If unassigned, they can only assign a new one. Hide "Remove Assignment"
        ...
        changeCourseForm.onsubmit = function(e) {
            ...
            if (newCourseSelect.disabled || newCourseSelect.value === "") {
                showCustomAlert('Error', 'Please select a new course to assign.');
                return;
            }
            ...
            showCustomConfirm(
                ...
                `Are you sure you want to change the course assignment to **${newCourseTitle}**? An email notification will be sent.`, 
                (confirmed) => {
                    if (confirmed) {
                        fetch('manage_mentors.php', {
                            method: 'POST',
        ...
        btnRemoveAssignment.onclick = function() {
            ...
            const currentCourseTitle = document.getElementById('currentCourseTitle').textContent;
            const mentorName = document.getElementById('mentorNameChange').textContent;
            
            if (!oldCourseId || currentCourseTitle === 'None Assigned' || currentCourseTitle === 'Unassigned') {
            ...
            showCustomConfirm(
                'Confirm Removal', 
                `Are you sure you want to **REMOVE** the course **${currentCourseTitle}** assignment for **${mentorName}**? An email notification will be sent.`, 
                (confirmed) => {
                    if (confirmed) {
