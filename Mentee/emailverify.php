...
$sql = "SELECT First_Name, Last_Name, Username, DOB, Gender, Email, Email_Verification, Contact_Number, Contact_Verification, Mentee_Icon 
        FROM mentee_profiles WHERE Username = ?";
    ...
    elseif (isset($_POST['verify_code'])) {
        ...
        if ($saved_code && time() - $timestamp <= 300) {
            if ($input_code == $saved_code) {
                $stmt = $conn->prepare("UPDATE mentee_profiles SET Email_Verification='Active' WHERE Username=?");
                $stmt->bind_param("s", $username);
                $stmt->execute();
...
<div class="sub-menu-wrap hide" id="profile-menu">
  <div class="sub-menu">
    ...
    <nav class="tabs">
      ...
      <button class="active" onclick="window.location.href='emailverify.php'">Email Verification</button>
